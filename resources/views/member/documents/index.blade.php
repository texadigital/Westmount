...
<div class="container mx-auto px-4 py-8">
    <div class="mb-8">
        ...
        <p class="mt-2 text-gray-600 dark:text-gray-400">Gérez vos documents téléchargés</p>
    </div>

    <div class="mb-6">
        <a href="{{ route('member.documents.create') }}" 
                        ...
                        <div class="flex items-center justify-between">
                            <div class="flex items-center">
                                ...
                                <div class="ml-4">
                                    ...
                                    <div class="text-sm text-gray-500 dark:text-gray-400">
                                        {{ $document->file_name }} • {{ $document->getFileSizeFormatted() }}
                                    </div>
                                    <div class="text-xs text-gray-500 dark:text-gray-400">
                                        Type: {{ MemberDocument::getDocumentTypes()[$document->type] }}
                                    </div>
                                </div>
                            </div>
                            <div class="flex items-center space-x-2">
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium
                                    ...
                                    @endif">
                                    {{ MemberDocument::getStatusOptions()[$document->status] }}
                                </span>
                                
                                <a href="{{ route('member.documents.download', $document) }}" 
                                ...
                                @if($document->status === 'pending')
                                    <form action="{{ route('member.documents.destroy', $document) }}" method="POST" class="inline">
                                        @csrf
                                        @method('DELETE')
                                        <button type="submit" 
    ...
    @else
        <div class="text-center py-12">
            ...
            <p class="mt-1 text-sm text-gray-500 dark:text-gray-400">Commencez par télécharger un document.</p>
            <div class="mt-6">
                <a href="{{ route('member.documents.create') }}" 
